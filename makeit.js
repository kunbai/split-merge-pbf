...
const PROCESSED_FILES = './processed.txt'
const SUPPORT_EXT = [
  '.mp4',
  '.avi',
  '.mkv',
...
const searchPBF = async function (dirPath) {
  ...
  for (let fileName of fileNames) {
    ...
    if (fstat.isDirectory()) {
      ...
      filePathList.push(path.join(inputPath, fileName))
    }
  }
...
const checkProcessed = async function (fileName) {
  // let fh = await fsPromise.open(PROCESSED_FILES, 'a+')
  let data = await fsPromise.readFile(PROCESSED_FILES, { encoding: 'utf-8', flag: 'a+' })
}
              ...
              (wcallback2) => {
                ...
                info.splitInfo.forEach((spInfo) => {
                  for (let i = 0, max = spInfo.repeat; i < max; i++) {
                    var listItemStr = "file '" + path.join(outputPath, spInfo.fileName) + "'\r\n"
